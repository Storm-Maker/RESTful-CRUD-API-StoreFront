...
export class DashboardQueries {
    ...
    async showCompletedOrdersByUser(user_id: string):Promise<string[]> {
        try {
    ...
    async showOrdersByUser(user_id: string):Promise<string[]> {
        try {
            const conn = await Client.connect();
    ...
    async showProductsByCategory(category: string):Promise<string[]> {
        try {
            const conn = await Client.connect();
    ...
    async showTopFiveProducts():Promise<string[]> {
        try {
            const conn = await Client.connect();
